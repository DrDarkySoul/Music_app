...
class talk_to_svr : public enable_shared_from_this<talk_to_svr>, boost::noncopyable
{
...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::string> mess = { "Привет",
                                        "Меня зовут",
                                        "Проверяем кодировку",
                                        "Иди ка отсюда",};
    ...
    for(int j = 0; j < 10; j++)
    {
        for( std::vector<std::string>::iterator iter = mess.begin(); iter < mess.end(); iter++)
        {
            talk_to_svr::start(ep, *iter);
